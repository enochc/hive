...
impl Hive {
    ...
    pub fn new_from_str(name: &str, properties: &str) -> Hive {
        ...
        let connect_to = match config.get("connect") {
            Some(v) => Some(String::from(v.as_str().unwrap()).into_boxed_str()),
            _ => None,
        };
    ...
    */
        ...
        while !self.sender.is_closed() {
            match self.receiver.next().await.unwrap() {
                SocketEvent::NewPeer { name, stream } => {
                    let p = Peer::new(name, stream, self.sender.clone());
                    if is_server {
    ...
    fn peer_string(&self) -> Option<String> {
        ...
        match self.listen_port.as_ref() {
            None => None,
            Some(port) => {
                let myname = String::from(self.name.as_ref());
                peers_string.push_str(&format!("{}|{}", myname, port));
