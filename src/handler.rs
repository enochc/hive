...
use crate::property::{property_to_sock_str, Property, PropertyType};
...
impl Handler {
    ...
    pub async fn send_property(&mut self, property: Property) {
        let message = property_to_sock_str(Some(&property), true).unwrap();
    ...
    pub fn hangup(&mut self) {
        block_on(self.sender.send(SocketEvent::Hangup { from: String::from("") })).expect("failed to hangup");
    }
}
